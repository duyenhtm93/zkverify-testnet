...
cat > circuits/product_greater_than.circom <<EOF
pragma circom 2.0.0;
include "../node_modules/circomlib/circuits/comparators.circom";

template ProductGreaterThan() {
    signal input a;
    signal input b;
...
component main = ProductGreaterThan();
EOF

circom circuits/product_greater_than.circom --r1cs --wasm --sym -o circuits/
...
snarkjs groth16 setup circuits/product_greater_than.r1cs pot12_final.ptau keys/product_greater_than.zkey
snarkjs zkey export verificationkey keys/product_greater_than.zkey keys/verification_key.json
...
for i in $(seq 1 $N); do
  ...
  snarkjs wtns calculate circuits/product_greater_than_js/product_greater_than.wasm input/input.json witness/witness.wtns
  snarkjs groth16 prove keys/product_greater_than.zkey witness/witness.wtns proofs/proof.json proofs/public.json
